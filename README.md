...
4) **Vote:** The group votes for or against the quest. The vote is a simple binary vote where the majority rules. Ties default to against. If it comes out against, repeat steps 1 and 3. (If this occurs three times in a row, the Evil players immediately gain 3 Victory Points.) **(Dev Note: If this doesn't end the game, there's no challenge getting to the fifth quest.)**
