...
Take each of the following steps during each round.
...
3) **Quest Proposal:** The Leader proposes a number of players for the next quest equal to the number written on the next quest on the board. Leaders can choose themselves for a quest.
...
Some abilities can be handled through the app, allowing a player to select a target and reveal information as needed. The ability "Exile" is handled in this way. In these cases, if the ability involves players leaving the room, they should still remove themselves. It is not necessary for players to show one another their phones, as sharing information will happen automatically through the app. Additional information, when applied mid-game, will be displayed as a norification. Known information can be reviewed at any time, even if forgotten by the player.
